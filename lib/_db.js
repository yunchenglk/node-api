...
var DB = function () {
    ...
    var filename = config.db.table_url;
 ...
 * @param {*} callback 回调方法
 */
DB.prototype.getPage = function (table_name, pageInfo, conditions, options, callback) {
    var node_model = this.getConnection(table_name);
    ...
    async.series([
        ...
        function (done) {
            ...
            node_model.find(conditions)
                .select(options.fields || '')
                .sort(options.sort || {})
                .skip((pageInfo.index - 1) * pageInfo.size)
                .limit(pageInfo.size)
                .exec(function (err, res) {
                    if (err) {
                        done(err, null);
    ...
    // var db = this;
    // var _callback = callback;
    // var count = this.count(table_name, conditions, function (err, res) {
    //     if (err) {
    //         callback(err);
    //     } else {
    //         if (res > 0) {
    //             var node_model = db.getConnection(table_name);
    //             node_model.find(conditions)
    //                 .select(options.fields || '')
    //                 .sort(options.sort || {})
    //                 .skip((pageInfo.index - 1) * pageInfo.size)
